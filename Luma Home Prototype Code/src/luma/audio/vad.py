...
def start_listening(callback: Callable[[str], None], stop_event=None, should_stop=None):
    ...
    def audio_callback(indata, frames, timing, status):
        if _speaking.is_set() or should_stop() or time.monotonic() < suppress_until: return
        try: q.put_nowait(indata[:, 0].copy())
        except queue.Full: pass

    with sd.InputStream(samplerate=SAMPLE_RATE, channels=1, dtype="int16", blocksize=_CHUNK, callback=audio_callback):
